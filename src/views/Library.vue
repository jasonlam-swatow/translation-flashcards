...
<template>
  <div class="p-4 max-w-2xl mx-auto">
    ...
    >
      <div class="bg-white p-4 rounded max-w-md w-11/12 modal-content">
        <p class="font-semibold mb-2" v-html="detail.text"></p>
        <p v-html="detail.translation"></p>
      </div>
    </div>
  </div>
</template>
...
<style scoped>
::v-deep(.modal-content b) {
  color: #3b82f6;
}
::v-deep(.modal-content rt) {
  color: orangered;
}
</style>
||||||| f817502
...
<template>
  <div class="p-4 w-screen mx-auto">
    ...
    <router-link
      to="/flashcards"
      class="btn mb-6 inline-flex items-center gap-1"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
    ...
    >
      <div class="w-full max-w-md h-64 bg-white shadow rounded p-4 mx-5 relative">
        ...
        <div class="h-full flex flex-col justify-center text-xl overflow-auto whitespace-pre-line">
          ...
          <div v-if="preview.note" class="mt-4 absolute bottom-4" style="width: calc(100% - 2rem)">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
            ...
            <span class="text-sm text-gray-600">{{ preview.note }}</span>
          </div>
        </div>
      </div>
    </div>
